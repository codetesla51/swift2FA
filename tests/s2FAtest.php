...
class s2FAtest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->Swift2FA = $this->getMockBuilder(Swift2FA::class)
            ->onlyMethods(["SendGridMail", "PHPMailer"]) // Mock email methods
            ->getMock();
    }
    ...
    public function testGenerateTOTP()
    {
        ...
        $timeStep = 130;
        $codeLength = 6;
        $generateCode = $this->Swift2FA->generateTOTP($secret, $timeStep, $codeLength);
    ...
    // Test mail functionality with SendGrid
    public function testMailSendGrid()
    {
        $email = "test@example.com";
        $message = "Test message";
        $name = "Test Name";
        ...
        $this->Swift2FA
            ->expects($this->once()) // Expect SendGridMail method to be called once
            ->method("SendGridMail")
            ->with($email, $message, $name, $subject)
            ->willReturn(true);

        // Assert that Mail method returns true when using SendGrid
        $result = $this->Swift2FA->Mail("SendGrid", $email, $message, $name, $subject);
        $this->assertTrue($result);
    }
    ...
    public function testInvalidMailType()
    {
        ...
        $this->Swift2FA->Mail("InvalidType", "test@example.com", "Test message", "Test Name", "Test Subject");
    }
}
